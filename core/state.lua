...
function State.new()
  ...
  function self:add_processed(item_name)
    self.processed_items[item_name] = true; return self
  end

  function self:clear_processed_items()
    self.processed_items = {}; return self
  end
  ...
  function self:gui_set_visible(visible)
    self.gui.visible = visible
    return self
  end

  function self:gui_is_visible()
    return self.gui.visible
  end

  -- Optional: GUI tab management
  function self:gui_set_tab(tab_name)
    self.gui.active_tab = tab_name
    return self
  end

  function self:gui_get_tab()
    return self.gui.active_tab
  end

  function self:gui_is_initialized()
    return self.gui.initialized
  end

  function self:gui_set_initialized(value)
    -- Could add validation/event triggering here
    if type(value) ~= "boolean" then
      error("gui_set_initialized requires boolean")
    end

    local old_value = self.gui.initialized
  ...
  function self:add_item_value(item_name, value)
    if value and value > 0 then
      ...
      self.profit_stats.items_by_name[item_name].total_value = self.profit_stats.items_by_name[item_name].total_value +
      value
    end
    return self
  end
  ...
  function self:add_destroyed_value(item_name, value)
    if value and value > 0 then
      ...
      self.profit_stats.items_by_name[item_name].destroyed = (self.profit_stats.items_by_name[item_name].destroyed or 0) +
      1
    end
    return self
  end
